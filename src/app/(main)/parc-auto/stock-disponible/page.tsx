...
/** Jours sur parc : priorité au champ joursSurParc renvoyé par l’API, sinon calcul depuis dateEntreeParc / dateEntryParc */
function getJoursSurParc(v: Vehicle): number | null {
  if (v.joursSurParc != null) return v.joursSurParc;
  const dateEntryParc = v.dateEntreeParc ?? v.dateEntryParc;
  if (!dateEntryParc) return null;
  const entry = new Date(dateEntryParc);
...
export default function StockDisponiblePage() {
  ...
  useEffect(() => {
    let cancelled = false;
    const natureParam = natureFilter === "depot" ? "DEPOT" : natureFilter === "transit" ? "TRANSIT" : natureFilter === "consommation" ? "CONSOMMATION" : natureFilter === "autres" ? "AUTRES" : undefined;
    vehiclesApi
      ...
      .catch((e) => {
        if (!cancelled)
          setError(e instanceof Error ? e.message : "Erreur chargement stock disponible");
      })
      .finally(() => {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-slate-500">
          Véhicules sur le parc, disponibles à la vente ou à la livraison. À ne pas confondre avec les véhicules régularisés (situation comptable clôturée) — voir le sous-menu « Véhicules régularisés » pour ceux-ci.
        </p>
      </div>
          ...
          <>
                    ...
                    return (
                      <tr key={v.id}>
                        ...
                        <td className="text-right">
                          <Link
                            href={`/parc-auto/${v.vin}`}
                            className="inline-flex font-medium text-accent-600 hover:text-accent-700 hover:underline"
                          >
                            VIN 360°
                          </Link>
                        </td>
            ...
            {vehicles.length === 0 && (
              <p className="py-12 text-center text-slate-500">
                Aucun véhicule en stock disponible.
              </p>
            )}
          </>
        )}
